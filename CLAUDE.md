...
and is the specification; `go/` is a full rewrite in Go, on the `go-port`
branch, with every command ported. Which one you are working on decides which
...
`go/` is a full rewrite, on the `go-port` branch. Every one of the 26 PHP
commands is ported. Its own README (`go/README.md`) is the document for it:
...
(`go/testdata_gen.php` and the `*_expect.php` scripts) and committed. Both
...
**The coverage floor is per package, not one number** (`go/coverage.sh`). This
...
`go/internal/invariant/` holds checks over the source itself, for properties no
...
The same rule holds in `go/`, where it is a test rather than a grep
